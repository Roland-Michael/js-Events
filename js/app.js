...
var menuItems = ['Big Mac', 'Quarter-Pounder', 'Cheeseburger'];

function valueMenu(){
    for(i = 0; i < menuItems.length; i++){
        console.log(menuItems[i]);
        menu.innerHTML = menuItems[i];
    }
}
...
Benjamin.addEventListener('click', quoteFunction);
...
random.addEventListener('click', randQuote);
...
function magic(){
    if(showmoney.style.display === "none"){
        ...
        showHide.innerHTML = "Hide the Secret";
    }else{
        showmoney.style.display = "none";
        showHide.innerHTML = "Get Secret Now";
    }
}
/*if(showmoney.innerHTML === false){
    catmoney.innerHTML = 
}else{
